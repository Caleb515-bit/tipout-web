import type { Metadata } from 'next';
import Link from 'next/link';
import { ArrowLeft, Clock } from 'lucide-react';

export const metadata: Metadata = {
  metadataBase: new URL('https://tipout.org'),
  title: 'How to Set Fair Role Weights for Bar and Floor Staff',
  description: 'A practical framework for balancing front-of-house multipliers between servers, bussers, barbacks, and bartenders without causing team friction.',
  openGraph: {
    title: 'How to Set Fair Role Weights for Bar and Floor Staff',
    description: 'A practical framework for balancing front-of-house multipliers.',
    url: 'https://tipout.org/blog/posts/how-to-calculate-role-weights',
    siteName: 'TipOut',
  },
};

export default function PostTwo() {
  return (
    <div className="flex flex-col min-h-screen p-5 bg-[#14171C] text-[#F2ECE4] font-sans max-w-md mx-auto relative pb-28">
      <div className="flex items-center space-x-4 mb-6 pt-2">
        <Link href="/blog" className="w-10 h-10 bg-[#1D2128] border border-[#2B303A] rounded-2xl flex items-center justify-center hover:bg-[#2B303A]/50 transition">
          <ArrowLeft className="w-5 h-5 text-[#F2ECE4]" />
        </Link>
        <span className="text-xs font-mono text-[#8B9099]">Back to blog</span>
      </div>

      <article className="space-y-6">
        <div>
          <div className="flex items-center space-x-2 text-xs font-mono text-[#8B9099] mb-3">
            <span>Sept 2026</span>
            <span>•</span>
            <span className="flex items-center space-x-1"><Clock className="w-3 h-3" /><span>4 min read</span></span>
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-[#F2ECE4] leading-snug">
            How to Set Fair Role Weights for Bar and Floor Staff
          </h1>
        </div>

        <div className="space-y-4 text-xs text-[#8B9099] leading-relaxed border-t border-[#2B303A] pt-4">
          <p className="text-sm text-[#F2ECE4] font-medium">
            Role weighting allows managers to assign percentage multipliers based on the operational value and physical demand of each shift position.
          </p>
          <p>
            Standard benchmarks usually set senior servers and bartenders at a 100% baseline weight. Supporting roles—such as barbacks or bussers—are scaled relative to their support requirements (e.g., 50% to 60%).
          </p>
          <h3 className="text-sm font-bold text-[#F2ECE4] pt-2">Keeping It Transparent</h3>
          <p>
            Never change role weights mid-week. Lock in your venue's configuration in your closing tool so staff can verify the breakdown instantly before leaving.
          </p>
        </div>
      </article>
    </div>
  );
}